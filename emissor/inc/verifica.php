...
/*function verificarNavegador(){
	if(navigator.appName=='Microsoft Internet Explorer'){
		alert('O sistema de nota fiscal eletr�nica n�o � compat�vel com o Internet Explorer. Recomendamos o Mozilla Firefox');
...
if($cod_seguranca == $_SESSION['autenticacao'] && $cod_seguranca){
	...
	if($sql->rowCount() > 0){ 
			...
			if(md5($txtSenha) == $dados['senha']){	
				if($dados['codtipo'] == 1){ 
					if(($dados['nfe'] == "s") || ($dados['nfe'] == "S")){
						...
						print("<script language=JavaScript>alert('O prestador referido n�o est� apto a emitir nfe, por favor verifique juntamente com a prefeitura a libera��o de nfe!');
		...
		print("<script language=JavaScript>alert('CPF/CNPJ ou C�digo n�o cadastrado no sistema! Favor verificar usu�rio.');parent.location='../login.php';</script>");
	} 
