...
<?php 
	...
	if($_GET['COD']){
		...
		WHERE
		  des.codigo = '$CODDES'");	
		list($codigo, $razaosocial, $cnpj, $cpf, $inscrmunicipal, $endereco, $competencia, $data_gerado, $total, $codverificacao)=$sql->fetch();
		$cnpj.=$cpf;
	}
	if($_GET['CODI']) {
		$CODDES = $_GET['CODI'];
		...
		");	
		list($codigo, $razaosocial, $cnpj, $cpf, $inscrmunicipal, $logradouro, $numero, $complemento, $competencia, $data_gerado, $codverificacao)=$sql->fetch();
		$cnpj.=$cpf;
