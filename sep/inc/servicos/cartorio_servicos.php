...
<?php
	...
	if($_POST['COD']){
		require_once("cartorio_servico_editar.php");
			...
			<table width="100%">
				<tr>
					...
					<select name="cmbCategorias" id="cmbCategorias" class="combo">
						<option value=""></option>
						<?php                
						$sql_categorias = $PDO->query("SELECT codigo, tipocartorio FROM cartorios_tipo ORDER BY tipocartorio");
						while(list($codigo,$tipocartorio) = $sql_categorias->fetch()){               
						?>
	...
	<?php
		if($_POST["btFiltro"] == "Pesquisar"){
			require_once("cartorio_servicos_busca.php");
		}
	}
	?>
	<div id="dvResult"></div>
