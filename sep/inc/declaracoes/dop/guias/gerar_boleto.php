...
list($boleto,$tipoboleto) = $sql->fetch();
	...
	for($i=0; $i<$cont; $i++){
		if($_POST["ckISS".$i]){
			...
			$PDO->query("INSERT INTO guias_declaracoes SET codguia='$codguia', codrelacionamento='$coddeclaracao[1]', relacionamento='dop_des'");
			$PDO->query("UPDATE dop_des SET estado='B' WHERE codigo='$coddeclaracao[1]'");
		}
	}
