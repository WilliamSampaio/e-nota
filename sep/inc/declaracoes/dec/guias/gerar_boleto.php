...
$sql=$PDO->query("SELECT bancos.boleto, 
				  boleto.tipo
				  FROM boleto
				  INNER JOIN bancos
...
for($i=0; $i<$cont; $i++){
	if($_POST["ckISS".$i]){
		...
		$PDO->query("INSERT INTO guias_declaracoes SET codguia='$codguia', codrelacionamento='$coddeclaracao[1]', relacionamento='cartorios_des'");
		$PDO->query("UPDATE cartorios_des SET estado='B' WHERE codigo='$coddeclaracao[1]'");
	}
}
