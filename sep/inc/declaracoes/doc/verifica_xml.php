...
<?php  
	   ...
	   $sql=$PDO->query("SELECT razaosocial FROM cadastro WHERE cnpj = '$opr_creditos' AND codtipo = '$codtipo'");
	   ...
	   if($sql->rowCount()>0){
		   ...
		   if($xml){
			   ...
			   {
					$sql=$PDO->query("SELECT conta FROM doc_contas");
					while(list($docconta)=$sql->fetch()){	
			   ...
			   if($verifica ==1)
			   {
				   ...
				   $sql=$PDO->query("SELECT codigo FROM cadastro WHERE cnpj = '$opr_creditos'");
				   list($codopr)=$sql->fetch();		   
				   $PDO->query("
						INSERT INTO 
							doc_des 
						SET 
				   ...
				   ");
				   $sql=$PDO->query("SELECT MAX(codigo) FROM doc_des WHERE codopr_credito = '$codopr'");
				   list($CodDocDes)=$sql->fetch();		   
				   foreach($xml ->CONTA as $conta)
				   {
						$sql=$PDO->query("SELECT conta FROM doc_contas");
						while(list($docconta)=$sql->fetch()){			
				 ...
				 $PDO->query("INSERT INTO guias_declaracoes SET codguia = '$CodGuia', codrelacionamento = '$CodDocDes', relacionamento = 'doc_des'");
				 */
