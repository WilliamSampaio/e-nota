...
elseif($valor == 'issretido'){	
	...
	if (mysql_num_rows($sql)>=1){
		...
		while(list($tomador,$valor,$multa,$competencia,$emissao,$est,$des_codigo)=$sql->fetch()){
...
elseif($valor == 'emissores'){
	...
	if(mysql_num_rows($sql)>=1)	{			  
	    ...
	    while(list($emissor,$competencia,$dataemissao,$total,$valor,$est)=$sql->fetch()){
			$guia_pago = $guia_pago=="S" ? "sim" : "não";
