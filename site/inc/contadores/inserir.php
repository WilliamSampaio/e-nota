...
while ($contsocios < $nrosocios) {
	if ($vetor_sociosnomes[$contsocios] != "") {
		if ($contsocios == 0) {
			$sql_cargo = $PDO->query("SELECT codigo FROM cargos WHERE cargo='responsavel'");
		} else {
...
alert('Contador cadastrado! Não esqueça de Imprimir o comprovante de cadastro que abrirá em uma nova janela!');
