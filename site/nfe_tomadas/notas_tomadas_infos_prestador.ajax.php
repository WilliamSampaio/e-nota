...
$sql_infos_prestador = $PDO->query("
	...
	");
$infos_prestador = $sql_infos_prestador->fetchObject();
...
if ($sql_infos_prestador->rowCount()) {
...
?>
	...
	");
			$numServicos = $sql_num_servicos->rowCount();
			if ($numServicos) {
			?>
				<input name="hdInputs" id="hdInputs" type="hidden" value="<?php echo $sql_num_servicos->rowCount(); ?>" />
				<?php
				$cont = 0;
				while ($dadosServicos = $sql_num_servicos->fetchObject()) {
				?>
				 ...
				 calculaISSNfeTomadas('hdInputs','<?php echo $cont; ?>');">
						...
						WHERE
							cadastro_servicos.codemissor = '{$infos_prestador->codigo}'
					");
									while ($servico = $sql_servicos->fetchObject()) { ?>
							...
							<td align="center">
								<input type="text" name="txtValorIssServico<?php echo $cont; ?>" id="txtValorIssServico<?php echo $cont; ?>" value="0,00" size="8" class="texto" readonly="readonly" />
							</td>
							<td align="center">
								<input type="text" name="txtISSRetidoManual<?php echo $cont; ?>" id="txtISSRetidoManual<?php echo $cont; ?>" size="8" class="texto" onkeyup="MaskMoeda(this);" onkeydown="return NumbersOnly(event);" onblur="calculaISSNfeTomadas('hdInputs','<?php echo $cont; ?>');" value="0,00" />
							</td>
						</tr>
						<tr>
						...
						<tr>
							<td colspan="5" align="center">
								<textarea name="txtDiscriminacaoServico<?php echo $cont; ?>" id="txtDiscriminacaoServico<?php echo $cont; ?>" cols="0" rows="0" style="width:95%; height:30px;"></textarea>
							</td>
						</tr>
